...
class TournamentConsumer(AsyncWebsocketConsumer):
    ...
    async def receive(self, text_data):
        ...
        if action == "connect":
        ...
        elif action == "add_all_players":
            await self.add_all_players(data)
            ...
            # else:
            #     games[game_id] = Game("Two Players (tournament)") # as default here
            #     await self.send(text_data=json.dumps({
            #         "type": "started",
            #         "game_id": game_id,
            #     }))
            ...
            print(f"making it here in end", flush=True)
    ...
    async def add_all_players(self, data):
        print(f"Player getting here in add_all_players")
        usernames = data.get("players", [])
        
        # Loop through the provided usernames and add each player
        for username in usernames:
            try:
                ...
                if user_id not in [p["id"] for p in self.tournament.players]:
                    self.tournament.add_player(user_id, username)
                    atournament = await sync_to_async(Tournament.objects.get)(id=self.tournament_db_id)
                    await sync_to_async(user.tournaments.add)(atournament)
                    print(f"Player {username} (ID: {user_id}) added to tournament.")
                else:
                    print(f"Player {username} (ID: {user_id}) already in tournament.")
                ...
                if not game.players:
                    ...
                    for player_id, player in game.players.items():
                        ...
                        if winner:
                            ...
                            game.reset_game("Two Players (tournament)") 
                            # reason = reason = _("Game Over: %(winner)s wins") % {'winner': winner}
                            # await self.channel_layer.group_send(
                            #     self.match_name,
                            #     {"type": "game.end", "reason": reason, "game_id": game_id}
                            #     # {"type": "game.end", "reason": f"Game Over: {winner} wins"}
                            # )
                            await self.channel_layer.group_send(
                                "tournament_lobby",
                                {"type": "game.result", "game_id": game_id, "winner": winner}
                            )
                            if game:
                                game.clear_game()
                            break
    ...
    async def game_created(self, event):
        ...
        updated = False
        for i, (gid, p1, p2) in enumerate(self.tournament.matches):
            if {p1, p2} == {player1, player2} and gid is None:
                self.tournament.matches[i] = (game_id, p1, p2)
                updated = True
                print(f"APPENED", flush=True)
                break

        # If no existing match found, append as new (fallback)
        if not updated:
            self.tournament.matches.append((game_id, player1, player2))
    ...
    #     game_id = event.get("game_id")  # Make sure event includes game_id
    ...
    #             game.clear_game()
    #     else:
    #         print(f"Game already cleared: {game_id}", flush=True)
    ...
    async def create_game_tournament(self, event):
        ...
        print(f"Game {game_id} created for {db_player1.username} vs {db_player2.username}.", flush=True)
    ...
    async def force_cache_update(self, event): # not needed anymore right?
            ...
            while game.running:
                ...
                if not game.running:
                    ...
                    if player_username and opponent_username:
                        ...
                        await self.tournament._start_next_match_in_queue() # to start next match
                    else:
