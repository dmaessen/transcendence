...
async def create_match(player_id):
    global player_queue
    User = get_user_model()  # Move this to the top of the function
...
#         except User.DoesNotExist:
#             # Fallback for guest users created in consumer
#             return User(id=player_id, username=f"Guest {abs(player_id)}")

#     player1 = await sync_to_async(get_user)(player1_id)
#     player2 = await sync_to_async(get_user)(player2_id)
